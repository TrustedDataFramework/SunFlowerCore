...
public class RuntimeTest {
    ...
    public static TestConfig[] getTestConfig(String filepath) throws Exception {
        return Start.MAPPER.readValue(
                FileUtils.getResource(filepath).getInputStream(),
                TestConfig[].class);
    }
    ...
    public void testSpecFunctions(String filename, Collection<String> functions, int skip, int limit) throws Exception {
        ModuleInstance instance = new ModuleInstance(
                ModuleInstance.Config.builder()
                        .binary(ByteStreams.toByteArray(FileUtils.getResource("testdata/spec/" + filename).getInputStream()))
                        .build()
        );
    ...
    public void testAddWasm() throws Exception {
        ModuleInstance instance = new ModuleInstance(
                ModuleInstance.Config.builder()
                        .binary(ByteStreams.toByteArray(FileUtils.getResource("expression-tests/add.wasm").getInputStream()))
                        .build()
        );
...
//        testSpecFunctions("switch.wasm", Collections.singleton("stmt"), 8, 1);
    ...
    public void testPersistent() throws Exception {
        ...
        System.out.println("============================> starting is "+new Date().toString());
...
// C:\Users\Sal\Documents\GitHub\consortium\wasmer\as-example\build
        String filename = System.getenv("FILE_PATH");
        if (filename == null || "".equals(filename.trim())) return;
        byte[] data = Bytes.concat("hello world!!!!".getBytes(StandardCharsets.UTF_8), new byte[]{0});
        ModuleInstance instance = new ModuleInstance(
        ...
        System.out.println(new String(instance.getResult(), StandardCharsets.UTF_8));
        System.out.println(instance.getGas());
    }
