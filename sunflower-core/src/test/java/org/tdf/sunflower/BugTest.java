...
public class BugTest {
    ...
    public static void main(String[] args) {
        ...
        byte[] root = accountTrie.getGenesisRoot().getBytes();
        ...
        for(int i = 0; i < iters; i++){
            ...
            for (String pk : pks) {
                for(int j = 0; j < count; j++){
                    HexBytes from = HexBytes.fromHex(pk);
                    ...
                    Transaction tx = Transaction
                            ...
                            .nonce(nonceCounter.get(pk).incrementAndGet())
                            .build();
                    txs.add(tx);
                }
            }
            ...
            b.setBody(txs);
            Trie<?, ?> t = accountTrie.update(root, b);
            root = t.commit();
            t.flush();
        }
    }
