...
public class VrfStateMachine {
     ...
     * <p>
     * Assume system try to transfer a 4M bytes block in p2p network: it is (4 *
     * 1024 * 1024) / (256 per transaction) = 16384 transactions, it is (16384
     * transactions) / (6 seconds) = 2730 tps.
     * <p>
     * Assume network is 50M bps and there are 3000 ms for p2p forwarding in a new
     * block transfer: it is (50 / 8) / 4 = 1.5625 p2p forwarding points, it is
     * totally 1.5625 * 4 = 6.25 p2p forwarding points.
     * <p>
     * Above calculated data is to tell us: If network is 50M bps and there are 3000
     * ms for p2p forwarding in a new block transfer, the max p2p forwarding points
     * of system should be 6, as blow: a -> b -> c -> d -> c -> d -> e
     * <p>
     * We think such p2p network topology can meet Consortium Blackchain
     * requirement: 1. 50M bps network speed 2. 2730 transaction per second 3. 6
     * layers of p2p network topology
     */
     ...
     * <p>
     * Every sorted committee member will broadcast 4 kinds of data received in Vrf
     * consensus: proposal proof, new block, reduction commit proof and final commit
     * proof
     * <p>
     * Averagely, VRFs function is sorting out number of committee members as
     * threshold specified for every role, totally number of received data is 4 *
     * threshold.
     * <p>
     * We also add number of 1 threshold as redundancy to reduce impact of VRFs
     * random attribute, so we set LRU max size to 5 * threshold.
     */
    ...
    public synchronized boolean addProof(ProposalProof proposalProof, boolean isFromPeer) {
        ...
        // Other logic is not allowed in VRF_STATE_INIT state, because we could not
        // check priority, etc.
    ...
    public synchronized boolean addProof(CommitProof commitProof, boolean isFromPeer) {
        if (isFromPeer) {
            // Check if proof owner is a registered validator
            if (!isRegisteredOwner(commitProof)) {
    ...
    private int forceTransit(final int newState) {
        // We should do transit even between same states, because of different VRF
        // Round.
    ...
    private boolean alignState(ProposalProof proof) {
        ...
        // because Proposer Proof is happened in VRF_STATE_PROPOSAL who is the startup
        // state,
    ...
    private void triggerTimeoutTransit(long timeout, int from, Timer timer) {
        // Create timer task and save it for cancelling timer task before submit a new
        // one
        timerTask = new StateTransitTimerTask(from, this, timer);
        timer.schedule(timerTask, timeout);
        if (DEBUG)
            logger.info("TimerTask is scheduled, timeout {}, from state {}", timeout, from);
    }
    ...
    private boolean isRegisteredOwner(ProposalProof proposalProof) {
    ...
    private boolean isRegisteredOwner(CommitProof commitProof) {
    ...
    private boolean isRegisteredOwner(VrfBlockWrapper newBlock) {
