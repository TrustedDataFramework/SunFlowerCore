package org.tdf.consortium.consensus.vrf;
...
public class HashUtil {
    ...
    static {
        ...
        EMPTY_LIST_HASH = sha3(RLPUtils.encodeList());
        EMPTY_TRIE_HASH = sha3(RLPUtils.encodeElement(EMPTY_BYTE_ARRAY));
    }
    ...
    public static byte[] calcNewAddr(byte[] addr, byte[] nonce) {
        ...
        byte[] encSender = RLPUtils.encodeElement(addr);
        byte[] encNonce = RLPUtils.encodeBigInteger(new BigInteger(1, nonce));

        return sha3omit12(RLPUtils.encodeList(encSender, encNonce));
    }

    /**
     * The way to calculate new address inside ethereum for {@link org.silkroad.vm.OpCode#CREATE2}
