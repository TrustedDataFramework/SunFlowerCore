...
import org.wisdom.consortium.vm.wasm.abi.Context;
import org.wisdom.consortium.vm.wasm.runtime.hosts.*;
import org.wisdom.consortium.vm.wasm.section.*;
...
public class ModuleInstance {
    ...
    public ModuleInstance(Config config) throws Exception{
        ...
        config.hostFunctions.add(new Payload(config.payload == null ? new byte[0] : config.payload));
        config.hostFunctions.add(new PayloadLength(config.payload == null ? new byte[0] : config.payload));

        if(config.withHosts){
            config.hostFunctions.addAll(Arrays.asList(new Abort(), new Log(), new Keccak256()));
            config.hostFunctions.addAll(new ContextHelper(config.context).getHelpers());
            config.hostFunctions.addAll(new Decimal().getHelpers());
            config.hostFunctions.addAll(new JSONHelper().getHelpers());
        }
        ...
        functionsMap.put(result.getName(), result);
